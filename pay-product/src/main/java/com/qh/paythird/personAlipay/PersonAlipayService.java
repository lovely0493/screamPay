package com.qh.paythird.personAlipay;
 ...
 */
@Service
public class PersonAlipayService {


	private static final Logger logger = LoggerFactory.getLogger(PersonAlipayService.class);
	...
	public R order(Order order) {
		...
		try {
			String code_url = PayService.paliQrUrl(order);
			...
			logger.info("个人支付宝 结束------------------------------------------------------");
		}
		
		return R.error("个人支付宝下单失败！");
	}
	...
	public R notify(Order order, HttpServletRequest request, String requestBody) {
		...
		logger.info("个人支付宝异步通知，sourceStr=" + sourceStr);
		logger.info("个人支付宝异步通知, localSign=" + localSign);
		logger.info("个人支付宝异步通知, sign=" + sign);
